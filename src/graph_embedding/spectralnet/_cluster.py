...
class SpectralNet:
    ...
    def predict(self, X: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            self.embeddings_ = self.embeddings_.detach().cpu()

        n = self.embeddings_.shape[0]
        dist = torch.from_numpy(np.zeros((n, n)))
        for idx in range(n):
            dis = torch.sum(torch.pow(self.embeddings_ - self.embeddings_[idx, :], 2), dim=1)
            dist[idx] = dis
